...
export default class AgentGittyx {
    ...
    constructor(limit: number, apiKey: string, model_name: string) {
        ...
        if (model_name) {
            ...
            this.router = new AgentRouter(apiKey, this.model_name);
            logging(`Using model: ${this.model_name}`);
        }
    }
    ...
    async summarizeCommits() {
        ...
        const toSummarize = this.cache.filter((c) =>
            !c.summary && !this.isTrivialCommit(c.message)
        );
    ...
    async generateOverallSummary() {
        ...
        const latestCommitDate = new Date(
            Math.max(...nonOverallCommits.map(c => new Date(c.date).getTime()))
        );
        ...
        if (overall && new Date(overall.date).getTime() >= latestCommitDate.getTime() && overall.numberOfCommits === this.limit) {
...
## Answer:
`;
        let chatHistory = loadSessionHistory(sessionId);
